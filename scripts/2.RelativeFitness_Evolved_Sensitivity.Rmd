...
## This file corresponds with Figshare analysis where a sensivity analysis was  
## perfomred withFACS data to see whether regating affected estimates of relative
## fitness depicted in Fig. 2
...
```{r}
png(filename="~/GitHub/MinimalCell/figures/4.figshare/1.rel_fitness.regate.png",
...
# Show Plot
img <- readPNG("~/GitHub/MinimalCell/figures/4.figshare/1.rel_fitness.regate.png")
...
```{r}
png(filename="~/GitHub/MinimalCell/figures/4.figshare/1.delta_rel_fitness.regate.png",
    width = 1200, height = 1200, res = 96*2) 
...
# Show Plot
img <- readPNG("~/GitHub/MinimalCell/figures/4.figshare/1.delta_rel_fitness.regate.png")
...
calcsheet2 <- mutate(calcsheet, T0_SOI_calculated = T0_SOI_raw - (T0_Cax_neg / 
          T0_Cax_time * T0_Sampling_time))
# Calc the T24 abundance of the SOI by subtracting background noise. Multiply by 100 to account for dilution difference.
calcsheet3 <- mutate(calcsheet2, T24_SOI_calculated = T24_SOI_raw - (T24_Cax_neg / 
          T24_Cax_time * T24_Sampling_time * 100))
# Calculate fitness over 24 hr relative to the labeled competitor strain
calcsheet4 <- mutate(calcsheet3, W_vs_mCherry_calculated = log(T24_SOI_calculated / 
          T0_SOI_calculated) / log(T24_mCherry / T0_mCherry))
# Divide by the avg fitness of the syn1.0 ancestor
calcsheet5 <- mutate(calcsheet4, W_vs_syn1.0anc_calculated = 
          W_vs_mCherry_calculated / 1.479025287)
# Divide by the avg fitness of the syn3B ancestor
calcsheet6 <- mutate(calcsheet5, W_vs_syn3Banc_calculated = 
          W_vs_mCherry_calculated / 0.69419982)
```
